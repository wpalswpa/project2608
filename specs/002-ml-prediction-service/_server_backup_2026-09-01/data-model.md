...
(diff14 등)──▶    model.joblib      전처리+추정기 파이프라인   ──▶  PredictionResult
 (읽기전용)       schema.json       입력 계약 14개
...
| View | 컬럼 | 쓰임 | 상태 |
|---|---|---|---|
| `v_lol_10min_diff14` | 16 | 학습·예측 기본 입력 | ✅ 적용·검증 완료 |
...
| `v_snapshot_features` | 18 | **시점별 학습·예측 입력** (`snapshotMinute` + split + 타깃 + 피처 14) | ✅ |
...
원천 1행 → 차이 지표 14개 + 정답 1개. **예측의 기본 단위이자 `schema.json` 이 계약하는 형태**다.
...
**순서가 계약의 일부다.** 위 14개의 나열 순서가 `schema.json` · 파이프라인 학습 · JS 산술의
...
**시점 의존성** — 이 14개는 `SNAPSHOT_MINUTE = 10` 의 정의다. 15분 확보 시
...
|---|---|
| `snapshot_minute` | 10 |
| `feature_order` | 14개 컬럼명 배열 — **순서가 곧 계약** |
| `features[]` | `{ name, dtype, min, max, description }` |
| `generated_at` · `source_rows` | 생성 일시 · 학습 표본 수 |

**불변식** — `len(features) == 14 == 학습 파이프라인의 입력 차원`. 이 등식이 SC-012이며,
...
| 필드 | 설명 | 소비처 |
|---|---|---|
| `feature_order` | 14개 순서 | 배열 인덱스 정합 |
| `coef[14]` · `intercept` | 로지스틱 파라미터 | 확률 계산 |
| `mean[14]` · `scale[14]` | StandardScaler 통계 | 정규화 |
...
|---|---|
| `id` | `M1`~`M7` |
| `dataset` | `diff14` `gold2` `diff13` `clean27` `clean29` `cluster5` |
