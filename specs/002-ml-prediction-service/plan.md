...
**모델 계열은 이미 확정되어 있다**(로지스틱 회귀 · 차이 지표 13개).
...
**설계의 중심 위험은 하나다** — 스택이 **예측 로직을 두 곳에 만들 수 있다**는 것. (9/1 Flask 단일 스택으로 확정해 이 위험 자체를 제거했다 — 웹이 `predict.py` 를 직접 import 한다.)
...
| **서버** | Flask (`web/app.py`) | ✅ 구현 완료 |
| **화면** | Jinja2 템플릿 (`web/templates/index.html`) | ✅ 구현 완료 |
...
| **입력** | 차이 지표 13개 (blue − red) | ✅ 확정 · View 로 구현됨 |
| **데이터셋** | gold2(2) · diff13(13) · diff13(14) · clean27(27) · clean29(29) · cluster5(중립 5축) | ✅ DB 에 존재 · ⬜ **DDL 16종 결손** |
...
| **2. 누수 불가능 설계** | ✅ | 입력 계약이 `schema.json` 13개로 닫혀 있고, **계약에 없는 키는 거부**한다(추가 항목 허용 안 함). 시점 이후 정보는 애초에 이름이 없어 실수로도 들어올 수 없다. 중복 제거도 별도 단계가 아니라 **차이 지표 생성의 부산물**이다 |
| **3. 비교는 통제된 조건에서만** | ✅ | 15분 비교는 후속 이관(A4)이라 이번 범위에서 비교 자체를 실행하지 않는다. 대신 diff13 vs clean29 대조군을 **동일 분할·동일 모델·동일 검증**으로 유지한다(`v_lol_10min_clean29` 가 그래서 존재한다) |
...
| 2. 기능 → 모델 → 피처 → 데이터 순서 | ✅ FR 에서 출발해 로지스틱 → 차이 지표 13개 → View 순으로 내려왔다. 성능 수치에서 역산하지 않았다 |
...
계획 v1.0 은 학습 입력 View 하나(`v_lol_10min_diff13`)를 기준으로 썼다. 그 뒤 EDA 가 진행되면서
...
| **diff13** ★ | `v_diff13_*` | 14 (blue−red) | **M1** 주 모델 | ⚠️ `v_lol_10min_diff13` 로 별도 정의 존재 (컬럼명 불일치) |
| **diff13** | `v_diff13_*` | 13 (diff13 − `EliteMonstersDiff`) | **M4** 정보량 통제 | ❌ 결손 |
...
| # | 불일치 | 조치 |
|---|---|---|
| **D1** | 같은 14피처가 `v_lol_10min_diff13`(`goldDiff`)와 `v_diff13_all`(`GoldDiff`) 두 이름 체계로 존재한다. 노트북이 `D14_MAP` 으로 매번 rename 하고 있다 | `features.py` 의 `feature_order` 를 **유일한 이름 출처**로 삼고, View 쪽을 여기에 맞춰 **한 벌로 통합**한다. rename 사전은 삭제한다 |
...
|---|---|---|---|---|---|---|
| **M1** ★ | 기본 예측 | LogReg + StandardScaler | diff13 | `model.joblib` `model_params.json` | `POST /api/predict` · 화면 승률·근거·경고 | FR-001~008 · 확정 |
...
| **M7** | 챌린저 (참조 전용) | RandomForest · HistGB | diff13 | `challenger.json` | **없음 — 문서에만** | 원리 4 근거 보존 |
...
| M2 | 12-2절 — 세 구성 벤치 | gold2 가 diff13 와 동률 (`ml_split` 0.7191 vs 0.7136) |
...
| 출처 | 데이터 | 시점 | 모델 | 보고 정확도 |
|---|---|---|---|---|
| **본 프로젝트** | 9,879경기 × 40컬럼 | **10분** | 로지스틱 (diff13) | **0.7136** (`ml_split` 홀드아웃) · CV 0.730~0.737 |
...
│   ├── compare.py                    🆕 M3·M4 구성 대조 (gold2·diff13·diff13·clean27·clean29)
...
│   ├── schema.json / schema.simple.json 입력 계약 13개 / 2개
...
│
├── web/                              ✅ Flask — 표시 계층 (app.py · test_parity.py)
│   └── src/
...
│
│   └── templates/index.html          ✅ 입력 폼 · 승률 막대 · 요인 표시
...
주요 결정: 차이 지표 13개의 유도 규칙(R1) · 시점 인자 구조(R2) · 웹 산술 이식(R3) ·
...
| [`schema.contract.json`](./contracts/schema.contract.json) | ✅ 예시가 13개·순서 일치 확인 |
...
| [`web-api.openapi.yaml`](./contracts/web-api.openapi.yaml) | ✅ YAML 파싱·Snapshot 13개가 `feature_order` 와 순서 일치 확인 |
...
| **더 단순한 대안을 왜 배제했나** | ① Express → `predict.py` 자식 프로세스 호출: 경로는 하나로 유지되나 요청마다 인터프리터 기동 비용 ② Python 예측 서버 프록시: 실행 계층이 늘어남. **→ 9/1 결론: Flask 로 가면 두 대안 모두 불필요하다.** 웹이 같은 프로세스에서 `predict.py` 를 import 하므로 경로가 하나이면서 기동 비용도 계층 추가도 없다 |
...
| **영향** | 초기화된 환경에서 **EDA 노트북이 실행되지 않는다** — `v_diff13_all` `v_gold2_*` `v_cluster5_all` 등을 찾지 못한다. 데이터 계층부터 재현한다던 v1.0 의 주장이 현재는 성립하지 않는다 |
