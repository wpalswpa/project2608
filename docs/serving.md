...
| `POST /api/summoner` | Riot ID 로 최근 솔로랭크 경기를 10분 시점 복기 (`count` 1~10 · `start` 로 페이지 · 결과를 5분 캐시하고 `cached` 로 알린다) (`RIOT_API_KEY` 필요, 없으면 503). 경기마다 `band`·`my_win_prob`·`my_won`·`verdict`·`played_at`·`top_factors`(5개), 전체 `summary`(판정별 건수·`period`) 포함 |
