...
| 사람 | 맡는 것 | 전달 방법 |
|---|---|---|
| **검증 담당** | 아래 6가지 점검 기준으로 훑어보기 · 안 읽히는 곳 찾기 | 확인 결과를 **메시지로 전달** → 팀장이 문서에 반영 |
...
| 6가지 기준 점검 | 검증 담당 | ⬜ 아래 표의 6항목을 훑어보고 걸리는 곳 전달 |

### 이 프로젝트를 판단하는 기준 6가지

발표와 제출은 아래 여섯 가지로 평가된다. 준비할 때도 이 기준만 본다.

| # | 기준 | 우리 상태 |
|---|---|---|
| 1 | **기능 → 모델 → 피처 → 데이터** 순서로 진행되었는가 | ✅ 무엇을 만들지 정하고(승률+이유+경고) → 모델 8종 비교 → 피처 13개 확정 → 데이터 검증 |
| 2 | 상관성을 분석해 **같은 절차를 한 번 더** 거쳤는가 | ✅ 페이즈 2 — 상관 히트맵·VIF로 중복 확인 → 구성 4종 재비교 → 13개 유지 결정 (`reports/phase2_feature_reduction.md`) |
| 3 | 추가 데이터로 10분 vs 15분 양상이 어떻게 바뀌었는가 | ✅ +5.25%p, 개선은 접전에만 (**빼도 프로젝트에 지장 없는 보너스**) |
| 4 | **배운 범위 안에서** 진행된 머신러닝 프로젝트인가 | ✅ scikit-learn 로지스틱·KMeans·PCA·교차검증 — 수업에서 다룬 것만 사용 |
| 5 | 발표자와 청중이 **이해할 수 있는 수준**인가 | ✅ 용어를 풀어 씀(홀드아웃 → 봉인한 시험지), 방어 카드 5장 |
| 6 | **서비스 구현이 잘 되는가** | ⬜ https://p4.sumzip.com 이 발표 순간까지 살아 있을 것 |

**검증 담당은 이 6가지를 기준으로** 문서와 화면을 훑어보고, 걸리는 곳을 팀장에게 전달한다.
특히 5번(이해되는가)이 사람만 판단할 수 있는 항목이다.
