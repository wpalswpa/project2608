...
| 사람 | 맡는 것 | 전달 방법 |
|---|---|---|
| **검증 담당** | **정성 스모크 테스트(SC-7)** · 문서·화면에서 안 읽히는 곳 찾기 | 확인 결과를 **메시지로 전달** → 팀장이 문서에 반영 |
...
| 문서 숫자 정합성 | 팀장 | ✅ **완료** — `factcheck` 0건 (문서 13개) |
| 페이즈 2 재정의 (골드 제외 재분석) | 팀장 | ✅ **완료** — 킬 계수 −0.107 → +0.508, 정확도는 0.002만 하락 |
| **문서 검증** (④) | 팀장 | ✅ **완료** — 문서마다 갈리던 계수를 실측으로 통일. 계수 대조 규칙을 `factcheck` 에 추가해 재발 차단 |
...
| **정성 스모크 테스트 (SC-7)** | 검증 담당 | ✅ 완료 — 3문항 중 2개 정답, 킬 오답을 근거로 README 부록 보완 (기록: docs/spec.md) |
